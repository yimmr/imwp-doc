...
一般在任务文件写添加到数据库的数据，但数据量大时这种写法不合适，此时应该把JSON格式的数据放到 `actions/data` 目录下的JSON文件，然后使用 `$manager->data()` 读取数据，参数是不含扩展名的文件名，更简便的方法是将不含扩展名的文件名写到属性 `data_files` 数组，框架自动加载这些数据，并用小驼峰式文件名做变量，文件名分隔符支持空格、短横、下划线。
...
静态类 `\Imon\WP\Demo\Multiple` 提供的方法可以把单个数据重复 n 个，任务文件中的代码不需拼接类名，类名已储存在变量 `$Multiple` ， 调用方法可以像这样写 `$Multiple::create()` 。

重复键值对数据使用 `create()` 方法，接受四个参数，顺序是原始数据数组、重复个数、替换规则数组、替换项数组。**索引从0开始，即第一条数据的键名是0。**

**替换规则**

使用一组规则替换指定数据，规则是数据键名对应回调的数组。键名支持点分隔语法，如 `2.title` 仅作用于第3条数据标题。

对应的回调支持两种类型，一种是自定义回调，一种是 `$Multiple` 类方法，直接写方法名即可，如 `'title'=>'affix'` 将调用 `$Multiple::affix($title, $index)` 并标题设为方法返回的值；此类型回调支持自定义参数，需将方法名改成数组并且跳过第一个参数，如 `'title' => ['method'=>'affix','params'=>[$arg2, $arg3]]` 。
...
|      名称     |                         参数                        |      返回值      | 说明                     |     |
| :---------: | :-----------------------------------------------: | :-----------: | ---------------------- | :-: |
|    create   | $data, $number = 10, $rules = \[], $replace = \[] |     array     | 本地文件添加到媒体库             |     |
|    affix    |         $string, $before = '', $after = ''        | string\|array | 附加前缀或后缀，可重复多条数据        |     |
| contentRand |             $content, $separator = ''             |     string    | 打乱数组再合并成字符串返回          |     |
|  termsRand  |              array $names, $depth = 1             |     array     | 随机取n个术语名称，再将他们随机用作父或子项 |     |

**$manager（部分）**

|    名称   |      参数      |  返回值  | 说明                |     |
| :-----: | :----------: | :---: | ----------------- | :-: |
|   data  |   $filename  | array | 加载JSON数据并转成数组返回   |     |
| logPost | $key, $value |  null | 记录新增的数据，实现数据追踪和删除 |     |
