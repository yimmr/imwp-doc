...
一般在任务文件写添加到数据库的数据，但数据量大时这种写法不合适，此时应该把JSON格式的数据放到 `actions/data` 目录下的JSON文件，然后使用 `$manager->data()` 读取数据，参数是不含扩展名的文件名，更简便的方法是将不含扩展名的文件名写到属性 `data_files` 数组，框架自动加载这些数据，并用小驼峰式文件名做变量。
